...
pub fn main() !void {
    ...
    try socket_address.in.format(stdout)
    try stdout.print("\n", .{});
    try stdout.flush();
...
pub fn read_request(conn: Connection,
    ...
    // looping, saves data from stream to buffer
    while (reader_interface.takeDelimiterExclusive('\n')) |line| {
        // End of HTTP request message
	if(std.mem.eql(u8, @as([]const u8, line), "\r")) break; // TODO: this Line is confusing why @as
    } else |_| {}
}
```
...
pub fn send_200(conn: Connection) !void {
    ...
    // buffer is not needed here, data is directly written to stream
    var writer = conn.stream.writer(&.{});
    const writer_interface = &writer.interface;
    _ = try writer_interface.write(message);
}
